...
class LLaMA:
    ...
    def build(
        ...
        if load_model:
            ...
            print(f"loaded checkpoint {ckpt_path} in time {time.time()-prev_time:.2f}s")
            prev_time = time.time()
    ...
    def text_completion(
        ...
        ), f"batch size must be less than {self.args.max_batch_size}"

        max_prompt_len = max(len(prompt) for prompt in prompt_tokens)
        assert (
            max_prompt_len <= self.args.max_seq_len
        ), f"batch size must be less than {self.args.max_seq_len}"
        ...
        for i, tokens_list in enumerate(prompt_tokens):
            tokens[i, : len(tokens_list)] = torch.tensor(tokens_list, dtype=torch.long, device=self.args.device)
        ...
        for _, curr_prompt_tokens in enumerate(tokens.tolist()):
